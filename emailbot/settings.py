...
def _float_env(name: str, default: float) -> float:
    """Унифицированный помощник: читает число с плавающей точкой из окружения."""

    try:  # Предполагаем штатный сценарий: переменная присутствует и валидна
        return float(os.getenv(name, str(default)))  # Преобразуем значение или значение по умолчанию к float
    except Exception:  # Перехватываем любые сбои преобразования
        return default  # Возвращаем безопасное значение, сохраняя стабильность настроек
...
HEARTBEAT_SEC: float = _get_float(  # Интервал в секундах между сигналами активности от бота
    "EMAILBOT_HEARTBEAT_SEC",  # Имя переменной окружения, позволяющей переопределить частоту heartbeat
    5.0,  # Значение по умолчанию: посылаем действие каждые пять секунд
)
TASK_NOPROGRESS_TIMEOUT_SEC: float = _get_float(  # Пороговое время ожидания прогресса длительных задач
    "EMAILBOT_TASK_NOPROGRESS_TIMEOUT_SEC",  # Имя переменной окружения для настройки тайм-аута отсутствия прогресса
    120.0,  # Значение по умолчанию: две минуты терпим «тишину» перед форсированным обновлением
)  # нижний предел «тишины» между пингами прогресса внутри задач

# === Встроенный watchdog (PTB JobQueue) ===
WATCHDOG_ENABLED: bool = os.getenv(
    "EMAILBOT_WATCHDOG_ENABLED", "1"
) == "1"  # По умолчанию включаем мягкий watchdog, чтобы ловить зависания
WATCHDOG_TIMEOUT_SEC: float = _float_env(
    "EMAILBOT_WATCHDOG_TIMEOUT_SEC",
    300.0,
)  # Сколько секунд допустимо отсутствия прогресса до предупреждения
WATCHDOG_ENFORCE_CANCEL: bool = os.getenv(
    "EMAILBOT_WATCHDOG_ENFORCE_CANCEL", "0"
) == "1"  # Отменяем задачи при зависании, только если администратор явно включил жёсткий режим
