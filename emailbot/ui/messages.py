...
def format_parse_summary(s: Mapping[str, object], examples: Iterable[str] = ()) -> str:
    ...
    suspicious_count = _as_int(  # Извлекаем количество подозрительных адресов (цифровой локал-парт)
        s.get("suspicious_numeric_localpart") or s.get("suspicious", 0)
    )
    if suspicious_count > 0:  # Добавляем строку только при наличии подозрительных адресов
        lines.append(f"🟡 Подозрительные: {suspicious_count}")  # Сообщаем, сколько адресов выглядит подозрительно
