...
# Short aliases without the legacy ``CB_`` prefix for новых обработчиков
EX_INIT: str = CB_EXAMPLES_INIT  # Объявляем современное имя колбэка открытия примеров
EX_BACK: str = CB_EXAMPLES_BACK  # Объявляем современное имя колбэка возврата к отчёту
EX_MORE_CD: str = CB_EXAMPLES_MORE_COOLDOWN  # Объявляем современное имя колбэка догрузки кулдауна
EX_MORE_FR: str = CB_EXAMPLES_MORE_FOREIGN  # Объявляем современное имя колбэка догрузки иностранных адресов
