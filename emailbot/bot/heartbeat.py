...
try:  # Пытаемся импортировать расширенный watchdog для глобальных отметок
    from emailbot import progress_watchdog as _watchdog  # Сообщаем глобальному watchdog о прогрессе
except Exception:  # Подстраховываемся на случай проблем с импортом
    _watchdog = None  # В тестовой среде допускаем отсутствие watchdog
...
class Heartbeat:
    ...
    async def _run(self) -> None:
        ...
        while not self._stop.is_set():  # Пока не поступил запрос на остановку
            ...
            if _watchdog is not None:  # Действуем только если watchdog успешно импортирован
                try:
                    _watchdog.touch("heartbeat")  # Отмечаем прогресс для внутреннего watchdog
                except Exception:
