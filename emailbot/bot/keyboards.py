...
from emailbot.ui.callbacks import (  # Импортируем константы колбэков для единообразия
    ...
    EX_INIT,  # Новое краткое имя для запуска примеров
)
...
def build_show_examples_button() -> InlineKeyboardMarkup:
    """Собрать одиночную кнопку показа примеров с новым callback."""

    builder = InlineKeyboardBuilder()  # Создаём билдер инлайн-клавиатур aiogram
    builder.button(  # Добавляем единственную кнопку вызова примеров
        text="👀 Показать примеры отфильтрованных",  # Формируем понятную подпись с эмодзи
        callback_data=EX_INIT,  # Используем новый идентификатор колбэка инициализации примеров
    )
    builder.adjust(1)  # Располагаем кнопку в одной строке для совместимости с UI
    return builder.as_markup()  # Возвращаем готовую клавиатуру в формате InlineKeyboardMarkup
